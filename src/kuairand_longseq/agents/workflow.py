    ...
    def run(self) -> Path:
        ...
        if readiness.facts.get("ready_for_human_approval"):
            ...
            self._move(
                ...
                RoleId.FEATURE_PUBLISHER.value,
                "Required Agent-admissible model evidence is unavailable; the Train-only GPU "
                "snapshot remains outside the active Validation contract.",
            )
        return self._finalize()
