...
GPU_SNAPSHOT = PROJECT_ROOT / "demo" / "gpu_train_only_snapshot"
EXPECTED_PROVENANCE = {
...
def test_gpu_train_only_snapshot_is_intact_but_not_claim_eligible() -> None:
    manifest = yaml.safe_load((GPU_SNAPSHOT / "evidence_bundle_manifest.yaml").read_text(encoding="utf-8"))
    verification = manifest["verification"]
    assert manifest["scope"]["partition_role"] == "train_only"
    assert manifest["scope"]["models"] == ["BL0", "BL1", "BL2"]
    assert verification == {
        "scientific_result_status": "numerically_verified",
        "agent_admission_status": "pending_provenance_completion",
        "claim_eligible": False,
        "release_eligible": False,
    }
    assert manifest["core_metrics"]["daily_direction_consistency"]["origins_improved"] == 7
    for artifact in manifest["artifacts"]:
        path = GPU_SNAPSHOT / artifact["path"]
        assert path.is_file()
        assert path.stat().st_size == artifact["size_bytes"]
        assert hashlib.sha256(path.read_bytes()).hexdigest() == artifact["sha256"]

    with (GPU_SNAPSHOT / "pooled_metrics.csv").open(encoding="utf-8", newline="") as handle:
        pooled = {row["model_id"]: row for row in csv.DictReader(handle)}
    expected = manifest["core_metrics"]["BL2_minus_BL1"]
    assert float(pooled["BL2"]["average_precision"]) - float(pooled["BL1"]["average_precision"]) == pytest.approx(
        expected["average_precision"], abs=1e-15
    )
    assert float(pooled["BL2"]["user_gauc_event_weighted"]) - float(
        pooled["BL1"]["user_gauc_event_weighted"]
    ) == pytest.approx(expected["user_gauc_event_weighted"], abs=1e-15)
    assert float(pooled["BL2"]["log_loss"]) - float(pooled["BL1"]["log_loss"]) == pytest.approx(
        expected["log_loss"], abs=1e-15
    )
    assert float(pooled["BL2"]["brier"]) - float(pooled["BL1"]["brier"]) == pytest.approx(
        expected["brier"], abs=1e-15
    )

    with (GPU_SNAPSHOT / "daily_metrics.csv").open(encoding="utf-8", newline="") as handle:
        daily_rows = list(csv.DictReader(handle))
    assert len(daily_rows) == 21
    by_origin: dict[str, dict[str, dict[str, str]]] = {}
    for row in daily_rows:
        by_origin.setdefault(row["origin"], {})[row["model_id"]] = row
    assert len(by_origin) == 7
    for models in by_origin.values():
        assert set(models) == {"BL0", "BL1", "BL2"}
        assert float(models["BL2"]["average_precision"]) > float(models["BL1"]["average_precision"])
        assert float(models["BL2"]["user_gauc_event_weighted"]) > float(
            models["BL1"]["user_gauc_event_weighted"]
        )
        assert float(models["BL2"]["log_loss"]) < float(models["BL1"]["log_loss"])
        assert float(models["BL2"]["brier"]) < float(models["BL1"]["brier"])
